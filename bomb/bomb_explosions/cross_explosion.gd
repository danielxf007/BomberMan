...
func explode( explosion_point: Tuple, matrix_of_cells: Array) -> Array:
	var left_explosion: Array = self.filter_explosions(
		matrix_of_cells, self.get_row_explosion(
		self.get_left_range(self.LEFT_RANGE, explosion_point),
		 explosion_point, matrix_of_cells)
	)
	var right_explosion: Array = self.filter_explosions(
		matrix_of_cells, self.get_row_explosion(
		self.get_right_range(self.RIGHT_RANGE, explosion_point),
		explosion_point, matrix_of_cells)
	)
	var up_explosion: Array = self.filter_explosions(
		matrix_of_cells, self.get_column_explosion(
		self.get_up_range(self.UP_RANGE, explosion_point),
		explosion_point, matrix_of_cells)
	)
	var down_explosion: Array = self.filter_explosions(
		matrix_of_cells, self.get_column_explosion(
